...
void alexaCallback(uint8_t id, bool state) 
{
  ...
  if (state) {  
    if (level == "100%") {
      ...
      digitalWrite(relayPin, LOW);
      espalexa.getDevice(0)->setValue(0);
      addLog("Alexa → ON BLOCKED (tank 100%)");
    } else {
...
// ======================================
// Alexa Server (LEGACY MODE, CORRECT)
// ======================================
...
void setup() 
{
  ...
  // Register Alexa device (do NOT start here)
