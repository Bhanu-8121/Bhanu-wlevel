/* Water Motor — FINAL (Option A)
   - Uses stable WiFiManager section from your sample code
   - Alexa (Espalexa) device registered in setup(), but espalexa.begin() is called
     only after a successful WiFi connection (Option A) to avoid captive-portal/port80 conflicts
   - OTA on port 81
   - Web serial log on port 82
   - Manual switch on D4 (GPIO2)
   - Auto ON at 0%, Auto OFF at 100%
   - Tank full lock for Alexa
   - Preserves all other modules and functionality
*/
...
// ==== Alexa ====
Espalexa espalexa;
bool espalexaStarted = false; // track whether espalexa.begin() was called
...
const int switchPin = 2; // D4 (will be wired to GND with INPUT_PULLUP)
...
void setupAlexaDeviceOnly()
{
    // Register the Alexa device — do NOT call espalexa.begin() here.
    // We'll call espalexa.begin() once WiFi is connected (to avoid port 80 conflict).
    espalexa.addDevice("Water Motor", alexaCallback);
    addLog("Alexa device registered: Water Motor (will start after WiFi)");
}
...
void setup()
{
    ...
    // LCD
    Wire.begin(0, 5); // SDA = 0 (D3?), SCL = 5 (D1) - adjust if needed
    ...
    // Register Alexa device now (but don't start it yet)
    setupAlexaDeviceOnly();
}
...
void loop()
{
    ...
    // --- WiFi/AP Manager Logic --- (from stable sample)
    ...
    logServer.handleClient();    // Web Log (serve even when offline so page can be viewed if connected locally)
    // Only call espalexa.loop() if espalexa.begin() was called (i.e., espalexaStarted)
    ...
    if (isConnected && !wifiOK) {
        ...
        // Start Espalexa now that WiFi is connected and port 80 is free
        if (!espalexaStarted) {
            espalexa.begin();
            espalexaStarted = true;
            addLog("Espalexa started (Alexa running)");
        }
